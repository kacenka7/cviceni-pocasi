...
predpoved.forEach(item=>{
    ...
    <div class="article-card">
      ...
      <p>${item.stav_pocasi}</p> 
      <a href="detail.html#${item.id}">Detail</a>
    </div>
  `
})
...
/*Rozdíl mezi těmi dvěma cvičeními (zprávičky a počasí) je ten, že zprávičky jsou seznam, ale počasí je objekt.

Objekt nepodporuje metodu forEach, ale přes jeho klíče jde iterovat například takto:

for (const den in predpoved) {
    const polozka = predpoved[den]
    console.log(polozka)
}
Nebo s použitím statické metody Object.entries a dekonstrukce pole:

Object.entries(predpoved).forEach(([den, polozka]) => {
	console.log(den)
	console.log(polozka)
})
*/
